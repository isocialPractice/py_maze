...
# the last, and three characters is what it takes for the exit column to
# stop falling left of the entrance column. Two wide puts the exit in
# column 0, left of the entrance rather than beside it, and one wide puts
# it in column -1, which is no column of the maze at all. At three the
# two meet in column 1, the one column a maze that narrow has that is not
# a border, and every width above it sets them apart
MIN_GRID_WIDTH = 3
...
def has_ends(grid):
    ...
    before the last, and :data:`MIN_GRID_WIDTH` is what it takes for the
    exit column to stop falling left of the entrance column. Two
    characters wide puts the exit in column 0, left of the entrance and
    on the border rather than beside it; one puts it in column -1, which
    Python reads from the other end of the row. At three the two meet in
    column 1, the one column a maze that narrow has that is not a border,
    and every width above it sets them apart. A maze straight from the
    generator is always wide enough, :data:`MIN_DIMENSION` cells carving
    five columns; a maze read out of a file is whatever the file drew,
    ...
    Returns:
        bool: True when the exit column is a column of the maze and not
        left of the entrance column, so everything that reads the two
        ends has ends to read
    """
