...
- [x] `Chaser.advance` reports steps the chaser did not take
  ...
  - From: Code Review Override - The Chase Settings Reached Off the Command
    Line
- [x] Put `wipe_rows` back in order in `py_maze/rendering.py`'s `__all__`
...
- [x] Say what `MazeGame.advance_chase` now answers with
  ...
  - From: Code Review Override - The Chase Point and the Interrupt's Ending
- [x] Settle a chaser's point the way 2.6.0 settled its speed
  ...
  - From: Code Review Override - The Chase Point and the Interrupt's Ending
- [x] **Goodbye Rows**: Print an interrupted game's goodbye into rows
  kept for it
...
- [x] The Windows Terminal escape hatch has never been read back on a
  Windows Terminal
...
- [x] The suite pins the dismissing goodbye at one console height, where
  ...
  - From: UI/UX Override - The Banners and the Dismissing Goodbye
- [x] `docs/playing.md` says the tallies stay where they are, and they move
  up two rows
  ...
  - **Goal**: say that the summary moves up by the rows the frame gives up
    ...
    keeps
  - From: UI/UX Override - The Banners and the Dismissing Goodbye
- [x] Nothing pins what a redirected run prints when an interrupt dismisses
  an ending
...
- [x] `can_display` reads every Windows terminal that is not Windows
  ...
  - From: Code Review Override - The Rows a Grown Console Gives Back
- [x] The dismissing sweep derives its tightest console but writes its span
  down
...
- [x] The swept frame-length assertion cannot fail
  ...
  - **Goal**: measure the frame rather than the slice - find the foot in
    ...
    checks
  - From: Code Review Override - The Override Turn With Nowhere to Put a GUI Test
- [x] **GUI Test Clause**: The override branch of
  `.claude/constants.md` names no file for a GUI test an override turn
  wants
